...
void xMD5Update(struct xMD5Context *context, UBYTE const *buf, int len);
void xMD5Final(byte digest[16], struct xMD5Context *context);
void xMD5Transform(ULONG buf[4], ULONG const in[16]);
 ...
 */
void
byteSwap(ULONG *buf, unsigned words)
{
    UBYTE *p = (byte *)buf;

    do {
        *buf++ = (ULONG)((unsigned)p[3] << 8 | p[2]) << 16 |
            ((unsigned)p[1] << 8 | p[0]);
        p += 4;
    } while (--words);
}
 ...
 */
void
xMD5Update(struct xMD5Context *ctx, UBYTE const *buf, int len)
{
    ULONG t;
...
xMD5Final(byte digest[16], struct xMD5Context *ctx)
{
    ...
    UBYTE *p = (byte *)ctx->in + count;  /* First unused byte */
 ...
 */
void
xMD5Transform(ULONG buf[4], ULONG const in[16])
{
...
void load_seed()
{
    ...
    fd = open(_md5digestname, O_RDONLY);
    if (fd >= 0)
    {
        read(fd, digest, 16);
        close(fd);
...
void save_seed()
{
    ...
    fd = open(_md5digestname, O_WRONLY|O_CREAT, 0600);
    if (fd >= 0)
    {
        write(fd, digest, 16);
        close(fd);
...
ULONG md5_rnd(ULONG low, ULONG high)
{
